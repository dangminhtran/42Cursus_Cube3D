...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/27 18:16:34 by dangtran          #+#    #+#             */
/*   Updated: 2025/04/27 18:17:17 by dangtran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_textures(t_game *game)
{
	...
	game->textures.north_img.img = mlx_xpm_file_to_image(game->window.mlx_ptr,
			game->textures.north_path, &game->textures.tex_width,
			&game->textures.tex_height);
	if (!game->textures.north_img.img)
		exit_error(game, "Failed to load north texture");
	game->textures.north_img.addr = mlx_get_data_addr(game->textures.north_img.img,
			&game->textures.north_img.bits_per_pixel,
			&game->textures.north_img.size_line,
			&game->textures.north_img.endian);
	game->textures.south_img.img = mlx_xpm_file_to_image(game->window.mlx_ptr,
			game->textures.south_path, &game->textures.tex_width,
			&game->textures.tex_height);
	if (!game->textures.south_img.img)
		exit_error(game, "Failed to load south texture");
	game->textures.south_img.addr = mlx_get_data_addr(game->textures.south_img.img,
			&game->textures.south_img.bits_per_pixel,
			&game->textures.south_img.size_line,
			&game->textures.south_img.endian);
	game->textures.east_img.img = mlx_xpm_file_to_image(game->window.mlx_ptr,
			game->textures.east_path, &game->textures.tex_width,
			&game->textures.tex_height);
	if (!game->textures.east_img.img)
		exit_error(game, "Failed to load east texture");
	game->textures.east_img.addr = mlx_get_data_addr(game->textures.east_img.img,
			&game->textures.east_img.bits_per_pixel,
			&game->textures.east_img.size_line,
			&game->textures.east_img.endian);
	game->textures.west_img.img = mlx_xpm_file_to_image(game->window.mlx_ptr,
			game->textures.west_path, &game->textures.tex_width,
			&game->textures.tex_height);
	if (!game->textures.west_img.img)
		exit_error(game, "Failed to load west texture");
	game->textures.west_img.addr = mlx_get_data_addr(game->textures.west_img.img,
